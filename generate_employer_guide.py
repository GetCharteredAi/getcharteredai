...
def page1(c):
    ...
    # ── HERO ────────────────────────────────────────────────────────────────
    y = H - 38 * mm  # extra breathing room below header
    c.setFillColor(NAVY)
    ...
    y = wrap(c,
        ...
        M, y, W - 2 * M, 'Helvetica', 10.5, SLATE, 6 * mm) - 8 * mm

    # ── STATS BAR ───────────────────────────────────────────────────────────
    bar_h = 28 * mm  # taller bar
    c.setFillColor(NAVY)
    ...
    for i, (num, lbl) in enumerate(stats):
        ...
        c.drawCentredString(cx, y - 13 * mm, num)
        c.setFillColor(MUTED)
        c.setFont('Helvetica-Bold', 8)
        c.drawCentredString(cx, y - 21 * mm, lbl)
        if i < 2:
            c.setStrokeColor(HexColor('#1e293b'))
            c.setLineWidth(0.5)
            c.line(col * (i + 1), y - bar_h + 4 * mm, col * (i + 1), y - 4 * mm)
    y -= bar_h + 14 * mm
    ...
    for bold, body in bullets:
        ...
        y -= 6 * mm
        y = wrap(c, body, M + 6 * mm, y, W - 2 * M - 6 * mm, 'Helvetica', 10, SLATE, 5.5 * mm) - 5 * mm
    y -= 8 * mm
    ...
    y -= 8 * mm
    c.setFillColor(NAVY)
    c.setFont('Helvetica-Bold', 15)  # larger heading
    ...
    wrap(c,
        ...
        M, y, W - 2 * M, 'Helvetica', 10.5, SLATE, 6 * mm)
...
def page2(c):
    ...
    progs = [
        ...
        dict(label='YEAR TWO PROFESSIONAL READINESS', price='£127', sub='one-off · standalone diagnostic',
    ...
    for i, p in enumerate(progs):
        ...
        lw = c.stringWidth(p['label'], 'Helvetica-Bold', 7)
        c.drawString(cx + (card_w - lw) / 2, ty, p['label'])
        ty -= 6 * mm
        # Price
        c.setFillColor(NAVY)
        c.setFont('Helvetica-Bold', 19)
        pw = c.stringWidth(p['price'], 'Helvetica-Bold', 19)
        c.drawString(cx + (card_w - pw) / 2, ty, p['price'])
        ty -= 6 * mm
        # Sub
        c.setFillColor(MUTED)
        c.setFont('Helvetica', 7.5)
        sw = c.stringWidth(p['sub'], 'Helvetica', 7.5)
        c.drawString(cx + (card_w - sw) / 2, ty, p['sub'])
        ty -= 6 * mm
        # Desc
        ty = wrap_centre(c, p['desc'], cx + card_w / 2, ty,
                         card_w - 6 * mm, 'Helvetica', 8.5, SLATE, 4.5 * mm) - 4 * mm
        # Divider
        c.setStrokeColor(HexColor('#e2e8f0'))
        c.setLineWidth(0.3)
        ...
        for feat in p['feats']:
            ...
            ty = wrap(c, feat, cx + 7 * mm, ty, card_w - 9 * mm,
                      'Helvetica', 8, SLATE, 4 * mm) - 1 * mm

    y -= card_h + 14 * mm

    # ── COHORT PRICING ────────────────────────────────────────────────────────
    band_h = 28 * mm  # taller band
    c.setFillColor(NAVY)
    c.rect(0, y - band_h, W, band_h, fill=1, stroke=0)
    c.setFillColor(GOLD)
    c.setFont('Helvetica-Bold', 7.5)
    c.drawCentredString(W / 2, y - 6 * mm, 'COHORT PRICING')
    c.setFillColor(white)
    c.setFont('Helvetica-Bold', 12)
    c.drawCentredString(W / 2, y - 13 * mm, 'Standard per-candidate pricing — no minimums.')
    c.setFillColor(MUTED)
    c.setFont('Helvetica', 9)
    c.drawCentredString(W / 2, y - 21 * mm,
        'All four programmes at the same price for every candidate. Enquire for a coordinated proposal.')
    y -= band_h + 10 * mm

    # CTA button
    btn_w, btn_h = 56 * mm, 10 * mm
    bx = (W - btn_w) / 2
    c.setFillColor(GOLD)
    c.roundRect(bx, y - btn_h, btn_w, btn_h, 2 * mm, fill=1, stroke=0)
    c.setFillColor(NAVY)
    c.setFont('Helvetica-Bold', 9.5)
    c.drawCentredString(W / 2, y - 6.5 * mm, 'Enquire about cohort access  →')
    y -= btn_h + 16 * mm
    ...
    circle_y = y
    r = 9 * mm  # larger circles
    for i, (num, head, body) in enumerate(steps):
        sx = M + i * (step_w + gap)
        ccx = sx + step_w / 2
        # Circle
        c.setFillColor(NAVY)
        c.circle(ccx, circle_y, r, fill=1, stroke=0)
        c.setFillColor(white)
        c.setFont('Helvetica-Bold', 12)
        nw = c.stringWidth(num, 'Helvetica-Bold', 12)
        c.drawString(ccx - nw / 2, circle_y - 4 * mm, num)
        # Heading
        ty = circle_y - r - 6 * mm
        c.setFillColor(NAVY)
        c.setFont('Helvetica-Bold', 10)
        ...
        ty -= 6 * mm
        # Body
        wrap_centre(c, body, sx + step_w / 2, ty, step_w - 4 * mm, 'Helvetica', 8.5, SLATE, 5 * mm)
        # Connector
        if i < 3:
            ...
            c.setLineWidth(0.5)
            c.setDash(3, 3)
            c.line(ccx + r, circle_y, nx - r, circle_y)
            c.setDash()
