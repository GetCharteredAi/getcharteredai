...
exports.handler = async (event) => {
  ...
  try {
    const { token, rows } = JSON.parse(event.body);
    if (!token || !Array.isArray(rows)) return { statusCode: 400, body: JSON.stringify({ success: false }) };

    const payload = verifyToken(token);
    const email = payload.email;

    const store = getStore({ name: 'recovery-plans', siteID: process.env.SITE_ID || process.env.NETLIFY_SITE_ID, token: process.env.NETLIFY_TOKEN || process.env.NETLIFY_ACCESS_TOKEN });

    await store.set(email, JSON.stringify(rows));
